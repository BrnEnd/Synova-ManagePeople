...
export default async function ManagementPage({ searchParams }: { searchParams: Promise<{ scope?: string }> }) {
  ...
  const cards = [
    ...
    { label: 'Horas não enviadas', value: hours(dashboard.notSubmittedMinutes), detail: scope === 'all' ? 'Todos os funcionários' : 'Sob minha gestão', href: `/gestao/competencias?scope=${scope}&status=not_submitted` },
    ...
    { label: 'Aguardando aprovação', value: dashboard.awaitingApproval, detail: 'Exigem sua análise', href: `/gestao/competencias?scope=${scope}&status=awaiting_approval` },
    { label: 'Aguardando Nota Fiscal', value: dashboard.awaitingInvoice, detail: 'Pendência do funcionário', href: `/gestao/competencias?scope=${scope}&status=awaiting_invoice` },
    { label: 'Aguardando pagamento', value: dashboard.awaitingPayment, detail: 'Pendência financeira', href: `/gestao/competencias?scope=${scope}&status=awaiting_payment` },
  ...
  return <main className="min-h-screen">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8 md:px-10 md:py-12">
      ...
      <div className="mt-6"><ScopeToggle basePath="/gestao" scope={scope} /></div>
