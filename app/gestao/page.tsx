...
export default async function ManagementPage() {
  ...
  const [employees, clients, pendingCompetencies] = await Promise.all([
    getEmployeesModule().list(identity.tenantId),
    getClientsModule().list(identity.tenantId),
    getApprovalsModule().listForManager(identity.tenantId, identity.id),
  ]);
  const cards = [
    ...
    { label: 'Aguardando aprovação', value: pendingCompetencies.length, detail: 'Competências atribuídas', href: '/gestao/competencias' },
    ...
    <main className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 md:px-10 md:py-12">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><p className="text-xs font-black uppercase tracking-[0.2em] text-orange-400">Visão Gestão</p><h1 className="mt-2 text-3xl font-black tracking-[-0.04em] text-white md:text-4xl">Painel operacional</h1><p className="mt-2 text-zinc-400">Acompanhe cadastros, clientes e competências que exigem sua ação.</p></div>
        ...
        <section className="mt-6 rounded-3xl border border-white/10 bg-zinc-900/65 p-6 md:p-8">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-500">Fluxo mensal</p>
          <h2 className="mt-3 text-2xl font-black text-white">Horas e aprovações conectadas</h2>
          <p className="mt-2 max-w-2xl leading-7 text-zinc-400">Funcionários lançam e enviam horas; o gestor responsável aprova ou devolve com um motivo auditável.</p>
        </section>
      </div>
    </main>
  );
