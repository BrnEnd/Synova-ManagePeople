...
type Review = {
  competence: {
    id: string; tenantId: string; employeeId: string; employeeName: string; clientName: string; managerName: string; managerUserId: string;
...
export function CompetenceReview({ review, blobEnabled, canManage, scope }: { review: Review; blobEnabled: boolean; canManage: boolean; scope: 'mine' | 'all' }) {
  ...
  return <div className="mx-auto max-w-6xl px-5 py-8 md:px-10 md:py-12">
    ...
    {canManage && review.competence.status === 'awaiting_approval' && <div className="mt-6 grid gap-6 lg:grid-cols-2">
    ...
    {canManage && review.competence.status === 'awaiting_payment' && <form className="mt-6 rounded-3xl border border-emerald-400/15 bg-emerald-400/5 p-6" onSubmit={pay}><h2 className="font-black text-white">Registrar pagamento</h2><p className="mt-2 text-sm text-zinc-400">Valor congelado: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format((review.competence.approvedAmountCents || 0) / 100)}</p><div className="mt-4 grid gap-3 sm:grid-cols-3"><label className="text-sm font-bold text-zinc-300">Data do pagamento<input className="field mt-2" name="paidDate" required type="date" /></label><label className="text-sm font-bold text-zinc-300">Comprovante<input accept="application/pdf,image/jpeg,image/png,image/webp" className="field mt-2" name="file" required type="file" /></label><label className="text-sm font-bold text-zinc-300">Observações<input className="field mt-2" maxLength={2000} name="notes" /></label></div><button className="button-primary mt-4" disabled={Boolean(busy)}>{busy === 'payment' ? 'Registrando…' : 'Confirmar pagamento'}</button></form>}
