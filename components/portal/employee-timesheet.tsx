...
import { isCompetenceEditable, MonthlyTimesheetGrid } from '@/components/portal/monthly-timesheet-grid';
...
type CompetenceData = {
  competence: { id: string; tenantId: string; employeeId: string; referenceMonth: string; clientName: string; managerName: string; status: CompetenceStatus; totalMinutes: number; adjustmentReason: string | null; forecastDocumentId: string | null; invoiceDocumentId: string | null };
  entries: Entry[];
};
...
export function EmployeeTimesheet({ detail, history, notifications, blobEnabled, payment }: { detail: CompetenceData; history: HistoryItem[]; notifications: Array<{ id: string; title: string; message: string; readAt: string | null; createdAt: string }>; blobEnabled: boolean; payment: { amountCents: number; paidAt: string; receiptDocumentId: string } | null }) {
  ...
  const [totalMinutes, setTotalMinutes] = useState(competence.totalMinutes);
  ...
  return <div className="mx-auto max-w-7xl px-5 py-8 md:px-10 md:py-12">
    <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-end">
      ...
      <div><div className="grid grid-cols-2 gap-3"><Metric label="Total lançado" value={hours(totalMinutes)} /><Metric label="Situação" value={statusLabels[competence.status] || competence.status} /></div>{editable && <><button className="button-primary mt-3 w-full" disabled={Boolean(busy) || totalMinutes === 0 || gridBlocked} onClick={submitCompetence} type="button">{busy === 'submit' ? 'Enviando…' : 'Enviar horas para aprovação'}</button>{gridBlocked && <p className="mt-2 max-w-80 text-xs text-amber-200">Conclua ou corrija as alterações do calendário antes de enviar.</p>}</>}</div>
    </div>
    ...
    <section className="mt-8 overflow-hidden rounded-3xl border border-white/10 bg-zinc-900/70">
      <div className="border-b border-white/10 px-5 py-5 sm:px-7"><h2 className="text-xl font-black text-white">Apontamento de horas</h2><p className="mt-1 text-sm text-zinc-500">Preencha vários dias em sequência. Use Enter para salvar e avançar.</p></div>
      <MonthlyTimesheetGrid competence={competence} initialEntries={entries} onBlockedChange={setGridBlocked} onTotalChange={setTotalMinutes} />
    </section>
