...
try {
  ...
  identification.set('file', new Blob(['%PDF-1.4\nE2E identification'], { type: 'application/pdf' }), 'identificacao.pdf');
  ...
  invoice.set('file', new Blob(['%PDF-1.4\nE2E invoice'], { type: 'application/pdf' }), 'nota-fiscal.pdf');
  ...
  receipt.set('file', new Blob(['%PDF-1.4\nE2E receipt'], { type: 'application/pdf' }), 'comprovante.pdf');
  const paid = await manager.request(`/api/management/competencies/${competenceId}/payment`, { method: 'POST', body: receipt });
